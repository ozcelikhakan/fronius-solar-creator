...
export class ProjectStateService {
  ...
  // Saves the Inverter step.
  saveInverter(inverter: InverterData) {
    this._project.update(p => (p ? { ...p, inverter } : p));
    const id = this._project()?.id;
    if (!id) return;

    return this.apollo.mutate({
      mutation: gql`
        mutation UpdateInverter($id: ID!, $inverter: InverterInput!) {
          updateProjectInverter(id: $id, inverter: $inverter) {
            id
          }
        }
      `,
      variables: { id, inverter }
    });
  }

  // Saves the Sizing step, including cable loss settings.
  saveSizing(sizing: SizingData) {
    this._project.update(p => (p ? { ...p, sizing } : p));
    const id = this._project()?.id;
    if (!id) return;

    return this.apollo.mutate({
      mutation: gql`
        mutation UpdateSizing($id: ID!, $sizing: SizingInput!) {
          updateProjectSizing(id: $id, sizing: $sizing) {
            id
          }
        }
      `,
  ...
  // Saves the Components step.
  saveComponents(components: ComponentSelection) {
    this._project.update(p => (p ? { ...p, components } : p));
    const id = this._project()?.id;
    if (!id) return;

    return this.apollo.mutate({
      mutation: gql`
        mutation UpdateComponents($id: ID!, $components: ComponentsInput!) {
          updateProjectComponents(id: $id, components: $components) {
            id
          }
        }
      `,
